...
void QuickSort(std::vector<Edge>::iterator begin,
    ...
    for (int i = 0; i < 2; i++) {
// #pragma omp task
        QuickSort(iter[2 * i], iter[2 * i + 1]);
    }
// #pragma omp taskwait
}

std::mutex g_mutex;
bool availThread=true;

void parallelQuickSort(std::vector<Edge>::iterator begin,
                       std::vector<Edge>::iterator end,
                       std::thread* const threadPool,
                       int* totalInd,
                       int numThreads) {
    auto cmp = [](const Edge &lhs, const Edge &rhs) {
        return lhs.weight < rhs.weight;
...
// #pragma omp task
        if (availThread){
            ...
            *totalInd+=1;
            int next=*totalInd;
            if(*totalInd>=numThreads) availThread=false;
            g_mutex.unlock();
            threadPool[next]=std::thread(parallelQuickSort,iter[2 * i], iter[2 * i + 1],threadPool,totalInd,numThreads);
        }else{
            QuickSort(iter[2 * i], iter[2 * i + 1]);
        }
...
// #pragma omp single nowait
        ...
        parallelQuickSort(edges.begin(), edges.end(),workers,&threadCount,numThreads);
        for(int i=1;i<numThreads;i++){
            if(workers[i].joinable()) workers[i].join();
        }
       
        std::cout << "The vector elements are : \n";
        // std::vector<Edge>::iterator ptr;
